...
// --- 辅助函数 (保持不变) ---
// ... (getProvider, getSigner, getContract, contractNotReady 保持不变)
...
try {
  ...
  console.warn('无法读取合约 ABI:', artifactPath);
}

function getProvider() { return new ethers.JsonRpcProvider(RPC_URL); }
function getSigner(provider) {
  if (!PRIVATE_KEY) return null;
  try { return new ethers.Wallet(PRIVATE_KEY, provider); } catch (err) { return null; }
}
...
function contractNotReady(res) {
  return res.status(500).json({ message: '合约未配置或 ABI 缺失。请检查 .env 文件 (PRIVATE_KEY, CONTRACT_ADDRESS)' });
}
// --- 辅助函数结束 ---


// --- Multer 配置 (保持不变) ---
// ... (uploadDir, metadataDir, storage, upload 保持不变)
...
// ... (GET /items/my-items/:address 保持不变)
router.get('/items/my-items/:address', async (req, res) => {
  ...
  try {
    ...
    res.status(200).json({ message: 'Successfully fetched user items', data: items });
  } catch (err) {
...
// --- POST /claim-db 路由 (保持不变) ---
router.post('/items/:id/claim-db', async (req, res) => {
  const { losterAddress } = req.body;
  const { id } = req.params; 

  if (!losterAddress) {
    return res.status(400).json({ message: 'Loster address is required' });
  }
  ...
  try {
    ...
    if (item.status === 'claimed') {
      return res.status(200).json({ message: 'Item already marked as claimed', data: item });
    }
    if (item.status === 'available') {
      return res.status(400).json({ message: '操作无效：必须先执行链下批准 (approve)，使物品状态变为 "pending_handover"' });
    }
    // (隐式) 只有 item.status === 'pending_handover' 才能继续

  } catch (dbErr) {
    return res.status(500).json({ message: 'DB query failed', error: dbErr });
  }
  ...
  try {
    ...
    return res.status(500).json({ message: 'Failed to query blockchain owner', error: chainErr });
  }

  // 3. 对比并更新 
  if (onChainOwner.toLowerCase() === item.finderAddress.toLowerCase()) {
    return res.status(403).json({ message: 'Verification failed: On-chain owner is still the finder.' });
  }
  if (onChainOwner.toLowerCase() !== losterAddress.toLowerCase()) {
    return res.status(403).json({ 
      message: 'Verification failed: On-chain owner does not match provided losterAddress.',
  ...
  try {
    ...
    res.status(200).json({ message: 'DB status verified and updated to claimed', data: updatedItem });

  } catch (err) {
    res.status(500).json({ message: 'Failed to update item status in DB', error: err });
  }
});
...
router.post('/items/:id/submit-claim', async (req, res) => {
  ...
  try {
    ...
    // [!! 核心修改 !!] 物品在 'pending_handover' 状态时，不允许新的申请
